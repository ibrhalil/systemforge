...
- **Karar:** **5 katmanlı referans mimari** (katman = sorumluluk; taahhüt seviyeleri aşağıda):
  ...
  **Faz 1 taahhüdü (katman 1+2):** `tenant/V3` (`parent_project_id` + `is_default` + tip başına partial unique default); `module/notes/V2` + `module/apps/V2` (`project_id` NOT NULL FK + backfill). Her not/özel uygulama bir konteynerde yaşar; tip başına tek "Genel" default (modül aktivasyonunda idempotent ensure). Nested API TaskController deseni (`/projects/{id}/notes|note-categories|custom-apps`); flat `/notes` `/custom-apps` cross-container filtre görünümü olarak kalır (`?projectId=` — backward compat). Guard'lar: içerik varken tip değişimi 409; parent döngüsü 409; default konteynerin tipi/parent'ı değişmez. Plan limitleri tenant-seviyesinde kalır; Faz 1'de agregasyon YOK.
...
- **Durum:** **TAMAMEN UYGULANDI (2026-08-28).** BE `SearchQueryArgumentResolver` (19 GET ucu `?sq=`; service-accounts paging/sort-only, records dışarıda) + FE codec/syncUrl/searchQueryGet wire-flip + tüm liste sayfalarında durum üçlüsü (skeleton/keep-previous/error+retry) + Drawer satır detayı + seçim/bulk-bar + CSV export + multi-sort + auto-refresh + saved views v1. Plan dosyası tamamlandı (silindi).
...
- **Revizyon 2 (2026-08-29, DataTable Hardening):** Dış denetim konsolidasyonu sonrası sağlamlaştırma: (1) `hiddenColumns` mutabakatı (`useMemo` stale elime, `hideable:false` asla gizlenmez, en az 1 kolon görünür, geçersiz density `normal`'e clamp); (2) seçim yaşam döngüsü satır anahtar imzasıyla (`data.map(rowKey).join('\0')` — aynı kümede auto-refetch seçim korur, sayfa/filtre/mod sıfırlar); (3) bulk onayında `pendingConfirm.rows` güncel `data` ile filtrelenir; (4) multi-sort zincir kolonları kendi ▲/▼ + `aria-sort` yönünü gösterir, inaktif sıralanabilirler `LuChevronsUpDown`; (5) kontrollü `viewMode` yerel state'e yazmaz, geçersiz kayıtlı mod `viewModes[0]`'a clamp, `errorIcon` prop; (6) otomatik hücre render'ı `cellText`'te; (7) tablo satırı `TableRow` `React.memo` bileşeni.
...
- **Karar:** (1) `QueryInspector` **salt-okunur** TanStack `QueryClient` cache event'leri üzerinden (`apiClient`'a dokunulmaz); her iki shell'de `import.meta.env.DEV ? lazy : null` (prod bundle'da tree-shake kanıtlı); provider'sız ortamda no-op. (2) Sıfır-bağımlılıklı `useVirtualList` saf pencere matematiği + DataTable'a `virtualized`/`rowHeight`/`scrollHeight`: **dahili sabit yükseklik scroll container** (kullanıcı kararı; sayfa-scroll sanallaştırma reddedildi), yalnız table view mode, satır yüksekliği tr'de zorlanır (math↔DOM birebir), sticky thead, spacer satırlar, `rowHeight` defaultu density'den (33/49/57), sayfa değişiminde pencere başa sarılır, viewport `clientHeight` (jsdom fallback `scrollHeight`). (3) Saved views **DB v2**: tenant **V6** — `t_saved_views` (`state` JSONB→String, expression unique `(user_id, storage_key, LOWER(name))` = v1 replace semantiği; FK→`t_users` CASCADE; `GeneratedIdAuditEntity` deseni); snapshot v2 opsiyonel `prefs` bloğu (hiddenColumns + density; columnOrder bilinçli dışlandı); apply **efemer** (localStorage'a yazılmaz); **sessiz v1→DB migrasyonu** (DB boş + v1 satır varsa POST, başarıda legacy key silinir). Yetkilendirme `iam:saved-view:read/write`; erişim kullanıcıya sıkı (`findByIdAndUserId` — yabancı id = 404), principalsız çağrı fail-closed.
...
### K-58
**CRUD yüzey kuralı — entity create/edit/view = sayfa; modal yalnızca listelenen istisnalar**
- **Bağlam:** Ekranlar üç farklı edit desenine bölünmüştü: users/notes sayfa-tabanlı, IAM geri kalanı (roles/groups/permissions) kısmen modal, projects/tasks/custom-apps/platform tamamen modal. Bu tutarsızlık hem UX hem geliştirici referansı açısından standardı kırıyordu. Endüstri hizalaması: Carbon (IBM) Modal Usage — modaller kısa/seyrek görevler içindir, tekrarlı işler ana sayfada, karmaşık akışlar tam sayfa; Ant Design — modal = sayfa geçişi olmadan hızlı etkileşim.
- **Karar:** (1) **Entity create/edit/view = SAYFA**: liste + `/new` + `/:id` aynı detail bileşenini çizer (`isCreate = !id`); edit sayfa-içi mod (draft `startEdit`'te BİR KEZ tohumlanır — refetch effect'ten asla; diff/sequential save; users deseni). (2) **Modal yalnızca:** yıkıcı onaylar (`ConfirmDialog`) · tek amaçlı hızlı yardımcılar (AssignRoles/Groups, ResetPassword, şirket status/plan değişimi, RawKey gösterimi) · kanban hızlı task create (kart tıklaması task sayfasına gider) · seyrek konfigürasyon formları (custom-app property/view) · request-logs Drawer inspector — istisna listesi KAPALI; yenisi DECISIONS kaydı olmadan eklenmez. (3) Kural `frontend/AGENTS.md`'de + `demo/patterns` canlı dokümantasyonunda öğretilir.
- **Uygulama (7 faz):** IAM roles+groups (`/new` route, `Edit*Modal` silindi, liste Assign modalları detail'deki inline section'a), permissions (aynı), projects (`/new` + **hiç olmayan proje düzenleme eklendi**), tasks (`/projects/:id/tasks/:taskId` sayfası; board modalı sadece quick-create), custom-apps (`apps/new` + app inline edit; record `apps/:id/records/new|:recordId` sayfası — tek-kayıt GET olmadığından kayıt verisi bounded records fetch'ten; kırık `/custom-apps/:id` linkleri `apps/:id`'ye düzeltildi), platform (`<a href>` → SPA `Link`, service-account create sayfası; RawKeyModal kaldı).
- **Kullanıcı kararları:** kapsam IAM+projects+tasks+custom-apps+platform; task edit tam sayfa (overlay değil); record da sayfaya taşındı; küçük yardımcı işlemler modal kalır; kanban quick-create + property/view + Drawer dokümante istisna.
- **Durum:** UYGULANDI (2026-09-06). FE lint temiz + 389/389 Vitest + prod build; e2e `module-crud.spec` yeni sayfa akışına güncellendi (jar rebuild sonrası koşulmalı). Plan dosyası tamamlandı → silindi.
- **Etki:** Tüm entity ekranları tek desende; yeni ekran referansı `UserDetailPage` + `DetailPagePatternDemo`. Silinen modallar: `EditRoleModal`, `EditGroupModal`, `CustomAppFormModal`, `RecordFormModal`, `CreateProjectModal`, `PermissionFormModal`, `CreateServiceAccountModal`, `AssignPermissionsModal`, `AssignGroupRolesModal`. Kullanılmayan i18n anahtarları (`*.newTitle`, `*.editing` vb.) budandı. Açık takip: e2e koşumu (jar rebuild gerekli).
...
- **Durum:** ÇÖZÜLDÜ (2026-08-25). `public/V3__token_hash_at_rest.sql` backfill (bekleyen linkler digest-lookup ile çalışmaya devam; consumed satırlar da hash'lenir → yeniden sunulan token `TENANT_TOKEN_ALREADY_USED`) + `admin_password_hash DROP NOT NULL`. `TokenHasher` utility (refresh store'lardaki private kopyalar buna birleştirildi); `verifyAndProvision` admin user sonrası `adminPasswordHash`'i null'lar (rollback'te geri döner — DEBT-10 retry hash'i bulur). `TokenPurgeJob` günlük 03:00 UTC, retention default 7 gün. (V3, 2026-08-27 consolidation'da `public/V1__baseline.sql`'e absorbs edildi.)
